...
- **Color usage rule (mandatory — every new UI must conform): see [`docs/color-usage.md`](docs/color-usage.md).** In short — page & top nav: `bg-surface` (now warm near-white `#FFF8F5`, **lighter** than cards); card/sheet/drawer/rail: `bg-surface-container-low`; nested block inside a card: `bg-surface-container`; modal/dialog/menu/tooltip: `bg-surface-container-high`; hover: one step up. Chips are outlined at rest and take `secondary-container` when selected. Filled buttons use the seed `bg-primary` #F56500 with a **16px semibold** white label — white is 3.11:1 on it, which is AA for large text only, so body-size brand text uses `text-primary-label` (P40, 6.16:1). Neutrals are warm: Oshap grey is gone. Never hardcode hex or use a raw palette step for a surface.
